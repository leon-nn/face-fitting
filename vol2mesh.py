...
for frame in np.arange(30, numFrames + 1):
    print(frame)
...
for i in range(param.shape[0]):
    ...
    exportObj(source, f = m.face, fNameOut = 'obama/shape/' + fName + '.obj')

'''
Orthographic projection to 2D image
'''
for i in range(param.shape[0]):
    fName = '{:0>5}'.format(i + 1)
    fNameImgScaled = 'obama/scaled/' + fName + '.png'
    source = generateFace(param[i, :], m)
#    gridZ = griddata(source[:2, :].T, source[2, :], (xv, yv), method = 'linear')
    
    imgScaled = mpimg.imread(fNameImgScaled)
    plt.figure()
    plt.imshow(imgScaled)
    plt.scatter(source[0, :], source[1, :], s = 0.1)
    plt.title(fName)
#    savefig('lm/lm' + fName + '.png', bbox_inches='tight')
#    plt.close('all')
